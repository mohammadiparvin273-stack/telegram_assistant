...
def init_db():
    with get_conn() as conn:
        ...
        c.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS subscription_until TEXT")
        c.execute("""
...
def extend_subscription(chat_id, days=30):
    with get_conn() as conn:
        c = conn.cursor()
        c.execute("SELECT subscription_until FROM users WHERE chat_id=%s", (chat_id,))
        row = c.fetchone()
        base = datetime.now()
        if row and row[0]:
            try:
        ...
        c.execute("UPDATE users SET subscription_until=%s WHERE chat_id=%s", (new_until, chat_id))
        conn.commit()
        return new_until
...
def create_payment_record(authority, chat_id, amount):
    with get_conn() as conn:
        c = conn.cursor()
        c.execute(
            "INSERT INTO payments (authority, chat_id, amount, created_at) VALUES (%s,%s,%s,%s)",
...
def get_payment(authority):
    with get_conn() as conn:
        c = conn.cursor()
        c.execute("SELECT chat_id, amount, status FROM payments WHERE authority=%s", (authority,))
        return c.fetchone()
...
def list_active_subscriptions():
    with get_conn() as conn:
        ...
        c.execute(
            ...
            "WHERE subscription_until IS NOT NULL AND subscription_until > %s ORDER BY subscription_until",
