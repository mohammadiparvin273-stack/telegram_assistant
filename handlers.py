...
def _parse_and_add_reminder(chat_id, raw_text):
    dt, remaining = persian_date.extract_datetime_and_text(raw_text)
    if not dt:
...
async def free_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تلاش برای تشخیص یادآوری با زبان طبیعی (بدون نیاز به فرمت خاص)
    if any(k in text for k in REMINDER_KEYWORDS):
        dt, remaining = persian_date.extract_datetime_and_text(text)
        if dt:
            title = remaining or "یادآوری"
            db.add_reminder(chat_id, title, dt.isoformat(timespec="minutes"))
            display = persian_date.to_jalali_display(dt.isoformat(timespec="minutes"))
