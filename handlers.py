...
همه‌چیز با دکمه کار می‌کنه، لازم نیست هیچ دستوری حفظ کنی. همه‌ی تاریخ‌ها هم به شمسیه و هر جوری که راحتی می‌تونی زمان رو بگی.

برای شروع بگو اسمت چیه؟"""

WELCOME_BACK = "خوش برگشتی 👋 از دکمه‌های پایین صفحه استفاده کن."

HELP = """راهنمای سریع 👇

همه‌ی کارها از طریق دکمه‌های پایین صفحه انجام می‌شه:
📌 یادآوری‌ها — ثبت/دیدن/حذف یادآوری
🗓 رویدادها — ثبت/دیدن/حذف قرارها
✅ کارها — لیست کارها با علامت انجام‌شده و حذف
🎯 اهداف — ثبت اهداف با ددلاین
🧠 حافظه — ثبت نکته، دیدن یا پاک کردن حافظه
📊 رشد و گزارش — نقشه راه شخصی و گزارش هفتگی
⚙️ تنظیمات — ساعت بریف روزانه، روز گزارش هفتگی، وضعیت اشتراک

اگه منو رو گم کردی، فقط دوباره بات رو باز کن یا یه پیام بفرست، منو خودش برمی‌گرده. لازم نیست فرمت خاصی رو رعایت کنی، هر جوری که راحتی بنویس، من خودم می‌فهمم."""
...
def _parse_and_add_reminder(chat_id, raw_text):
    dt, remaining, method = persian_date.extract_datetime_and_text(raw_text)
    if not dt:
        return None, "زمانش رو متوجه نشدم 🙏 یه بار دیگه بگو، مثلا: «فردا ساعت ۹ تماس با علی» یا «1405/05/10 14:00 جلسه»"
    title = remaining or "یادآوری"
    db.add_reminder(chat_id, title, dt.isoformat(timespec="minutes"))
    display = persian_date.to_jalali_display(dt.isoformat(timespec="minutes"))
    return dt, f"✅ یادآوری ثبت شد برای {display}:\n{title}"


def _parse_and_add_event(chat_id, raw_text):
    dt, remaining, method = persian_date.extract_datetime_and_text(raw_text)
    if not dt:
        return None, "زمانش رو متوجه نشدم 🙏 یه بار دیگه بگو، مثلا: «سه‌شنبه بعد ساعت ۱۰ جلسه کاری» یا «1405/05/10 14:00 جلسه»"
    title = remaining or "رویداد"
    db.add_event(chat_id, title, dt.isoformat(timespec="minutes"))
    display = persian_date.to_jalali_display(dt.isoformat(timespec="minutes"))
    return dt, f"📅 رویداد ثبت شد: {title} در {display}"
...
def _parse_and_add_goal(chat_id, raw_text):
    ...
    title = parts[0]
    if not title:
        return "فرمت: عنوان هدف | ددلاین(اختیاری) | توضیح(اختیاری)"
    deadline_raw = parts[1] if len(parts) > 1 else ""
    description = parts[2] if len(parts) > 2 else ""
    deadline_iso = None
    if deadline_raw:
        d = persian_date.parse_flexible_date_only(deadline_raw)
        if d:
            deadline_iso = d.isoformat()
    goal_id = db.add_goal(chat_id, title, description, deadline_iso)
    reply = f"🎯 هدف #{goal_id} ثبت شد: {title}"
    if deadline_iso:
        reply += f" (ددلاین: {persian_date.to_jalali_display(deadline_iso, with_time=False)})"
    return reply
...
async def _send_reminders_list(target, chat_id):
    ...
    if not rows:
        ...
        return
    lines = [f"{persian_date.to_jalali_display(t)} | {text}" for r_id, text, t in rows]
...
async def _send_events_list(target, chat_id):
    ...
    if not rows:
        ...
        return
    lines = [f"{persian_date.to_jalali_display(t)} | {title}" for e_id, title, t, desc in rows]
...
async def _send_goals_list(target, chat_id):
    ...
    for g_id, title, desc, deadline in rows:
        line = f"🎯 {title}"
        if deadline:
            line += f" (تا {persian_date.to_jalali_display(deadline, with_time=False)})"
        lines.append(line)
...
def is_access_allowed(chat_id):
    ...
    if trial_start:
        try:
            if datetime.fromisoformat(trial_start) + timedelta(days=config.TRIAL_DAYS) > now:
                return True
        except ValueError:
            pass
    return False
...
def _subscription_status_text(user):
    now = persian_date.now_tehran()
    sub_until = user.get("subscription_until")
    if sub_until:
        try:
            ...
            if until_dt > now:
                return f"✅ اشتراکت فعاله تا {persian_date.to_jalali_display(sub_until, with_time=False)}"
        except ValueError:
            pass
    trial_start = user.get("trial_start")
    if trial_start:
        try:
            trial_end = datetime.fromisoformat(trial_start) + timedelta(days=config.TRIAL_DAYS)
            if trial_end > now:
                remaining = trial_end - now
                days_left = max(1, remaining.days + 1)
                return f"🎁 توی دوره‌ی آزمایشی رایگانی، {days_left} روز دیگه مونده."
        except ValueError:
            pass
    return "⛔ دوره‌ی آزمایشی رایگانت تموم شده."


async def _send_paywall(target, chat_id):
    user = db.get_user(chat_id)
    status_text = _subscription_status_text(user)
    price_toman = config.SUBSCRIPTION_PRICE_TOMAN
    price_rial = price_toman * 10
    await target.reply_text(
        f"{status_text}\n\n"
        f"برای فعال‌سازی اشتراک ماهانه ({price_toman:,} تومان / {price_rial:,} ریال)، "
        f"مبلغ رو به این حساب واریز کن:\n\n"
        f"💳 شماره کارت: {config.BANK_CARD_NUMBER}\n"
        f"🏦 شبا: {config.BANK_SHABA}\n"
        f"👤 به نام: {config.BANK_CARD_NAME}\n\n"
        f"بعد از واریز، عکس رسیدت رو همراه با این کد به پشتیبانی (@{config.ADMIN_TELEGRAM_USERNAME}) بفرست:\n\n"
        f"🆔 کد شما: {chat_id}\n\n"
        f"⏳ اشتراکت معمولاً ظرف ۱۲ تا ۲۴ ساعت فعال می‌شه."
    )


async def _send_subscription_status(target, chat_id):
    user = db.get_user(chat_id)
    if is_access_allowed(chat_id):
        await target.reply_text(_subscription_status_text(user))
        return
    await _send_paywall(target, chat_id)
...
async def subscribe_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        ...
        return
    await _send_subscription_status(update.message, chat_id)


# ==================== پنل مدیریت (فقط برای خودت) ====================
def _is_admin(chat_id) -> bool:
    return bool(config.ADMIN_CHAT_ID) and str(chat_id) == str(config.ADMIN_CHAT_ID)


async def admin_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    if not _is_admin(chat_id):
        return  # کاربر عادی هیچ واکنشی نمی‌بینه
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("➕ فعال‌سازی اشتراک با کد کاربر", callback_data="admin:activate")],
        [InlineKeyboardButton("📋 لیست اشتراک‌های فعال", callback_data="admin:list")],
    ])
    await update.message.reply_text("🔑 پنل مدیریت", reply_markup=kb)


async def _admin_list_active(target):
    rows = db.list_active_subscriptions()
    if not rows:
        await target.reply_text("هیچ کاربر مشترکی فعلاً فعال نیست.")
        return
    lines = [
        f"🆔 {chat_id} | {name or '(بی‌نام)'} | تا {persian_date.to_jalali_display(until, with_time=False)}"
        for chat_id, name, until in rows
    ]
    await target.reply_text("\n".join(lines))
...
async def _send_roadmap(target, chat_id):
    ...
    goals_text = "\n".join(
        f"- {g[1]}" + (f" (تا {persian_date.to_jalali_display(g[3], with_time=False)})" if g[3] else "")
        for g in goals
    )
...
async def remind_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    _, reply = _parse_and_add_reminder(chat_id, " ".join(context.args))
...
async def event_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    _, reply = _parse_and_add_event(chat_id, " ".join(context.args))
    await update.message.reply_text(reply)
...
async def goal_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not raw:
        await update.message.reply_text(
            "فرمت: /goal عنوان هدف | ددلاین(اختیاری) | توضیح(اختیاری)\n"
            "مثال: /goal یادگیری زبان انگلیسی سطح B2 | سه ماه دیگه | برای مکالمه کاری"
        )
        return
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("admin:"):
        ...
        if data == "admin:activate":
            ...
            await query.message.reply_text("کد کاربری (chat_id) که مشتری برات فرستاده رو بفرست:")
        elif data == "admin:list":
            await _admin_list_active(query.message)
        return
    ...
    if not is_access_allowed(chat_id):
        await _send_paywall(query.message, chat_id)
        return

    if data == "m:remind_add":
        context.user_data["awaiting"] = "remind_add"
        await query.message.reply_text("کی و چی رو یادآوری کنم؟ (مثلا: فردا ساعت ۹ تماس با علی)")
        ...
        await query.message.reply_text("زمان و عنوان رویداد رو بگو (مثلا: سه‌شنبه بعد ساعت ۱۰ جلسه کاری)")
        ...
        await query.message.reply_text("فرمت بفرست: عنوان هدف | ددلاین(اختیاری) | توضیح(اختیاری)")
    ...
    elif data == "m:subscribe":
        await _send_subscription_status(query.message, chat_id)
    elif data == "m:mycode":
        await query.message.reply_text(f"🆔 کد کاربری تو: {chat_id}")
...
async def free_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # پنل مدیریت: فعال‌سازی اشتراک با کد کاربر
    if context.user_data.get("awaiting") == "admin_activate" and _is_admin(chat_id):
        context.user_data["awaiting"] = None
        try:
            target_chat_id = int(persian_date.normalize_digits(text).strip())
        except ValueError:
            await update.message.reply_text("کد نامعتبره، باید یه عدد باشه.")
            return
        target_user = db.get_user(target_chat_id)
        if not target_user:
            await update.message.reply_text("کاربری با این کد پیدا نشد (باید حداقل یه بار /start زده باشه).")
            return
        new_until = db.extend_subscription(target_chat_id, days=config.SUBSCRIPTION_DAYS)
        display_until = persian_date.to_jalali_display(new_until, with_time=False)
        await update.message.reply_text(f"✅ اشتراک کاربر {target_chat_id} تا {display_until} فعال شد.")
        try:
            await context.bot.send_message(
                chat_id=target_chat_id,
                text=f"🎉 اشتراکت فعال شد! تا {display_until} می‌تونی از همه‌ی امکانات استفاده کنی.",
            )
        except Exception:
            pass
        return
    ...
    if awaiting and user:
        ...
        if awaiting == "remind_add":
            dt, reply = _parse_and_add_reminder(chat_id, text)
            context.user_data["awaiting"] = None if dt else "remind_add"
        elif awaiting == "event_add":
            dt, reply = _parse_and_add_event(chat_id, text)
            context.user_data["awaiting"] = None if dt else "event_add"
            ...
            context.user_data["awaiting"] = None
            try:
                hour = int(persian_date.normalize_digits(text))
                assert 0 <= hour <= 23
                db.set_daily_hour(chat_id, hour)
            ...
            context.user_data["awaiting"] = None
            try:
                day = int(persian_date.normalize_digits(text))
                assert 0 <= day <= 6
                db.set_weekly_day(chat_id, day)
    ...
    if not is_access_allowed(chat_id):
        await _send_paywall(update.message, chat_id)
        return
    ...
    # تلاش برای تشخیص یادآوری با زبان طبیعی (بدون نیاز به کلمه‌ی خاص)
    dt, remaining, method = persian_date.extract_datetime_and_text(text)
    has_keyword = any(k in text for k in REMINDER_KEYWORDS)
    if dt and (method == "strong" or has_keyword):
        title = remaining or "یادآوری"
        db.add_reminder(chat_id, title, dt.isoformat(timespec="minutes"))
        display = persian_date.to_jalali_display(dt.isoformat(timespec="minutes"))
        await update.message.reply_text(f"✅ یادآوری ثبت شد برای {display}:\n{title}")
        return
    if has_keyword and not dt:
        await update.message.reply_text("زمانش رو دقیق متوجه نشدم 🙏 یه بار دیگه بگو، مثلا: «فردا ساعت ۹ یادم بنداز تماس بگیرم»")
        return
