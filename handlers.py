...
async def _send_paywall(target, chat_id):
    ...
    if not config.ZARINPAL_MERCHANT_ID:
        await target.reply_text(
            f"{status_text}\n\nبرای ادامه استفاده باید مشترک بشی، ولی درگاه پرداخت هنوز توسط مدیر ربات تنظیم نشده."
        )
        return

    callback_url = f"{config.WEBHOOK_URL}/zarinpal/callback"
    result = zarinpal_module.request_payment(
        config.SUBSCRIPTION_PRICE_TOMAN,
        "اشتراک ماهانه دستیار شخصی هوشمند",
        callback_url,
    )
    if not result["ok"]:
        await target.reply_text(
            f"{status_text}\n\n⚠️ مشکلی در اتصال به درگاه پرداخت پیش اومد. کمی بعد دوباره امتحان کن."
        )
        return

    db.create_payment_record(result["authority"], chat_id, config.SUBSCRIPTION_PRICE_TOMAN)
    sandbox_note = "\n\n(الان در حالت آزمایشیه، پول واقعی کم نمی‌شه)" if config.ZARINPAL_SANDBOX else ""
    await target.reply_text(
        f"{status_text}\n\n"
        f"برای ادامه، اشتراک ماهانه {config.SUBSCRIPTION_PRICE_TOMAN:,} تومان رو از لینک زیر پرداخت کن:\n"
        f"{result['url']}{sandbox_note}"
    )
