...
async def check_reminders(bot):
    now_iso = persian_date.now_tehran().isoformat(timespec="minutes")
...
async def hourly_broadcast(bot):
    """هر ساعت اجرا می‌شه؛ برای کاربرانی که ساعتشون با الان (به وقت تهران) مطابقت داره، بریف روزانه
...
async def send_daily_brief(bot, chat_id, name, specialty):
    today = persian_date.now_tehran().date().isoformat()
