...
# ترتیب مهمه: اسم‌های ترکیبی (که خودشون شامل «شنبه» هم هستن) باید قبل از «شنبه» ساده چک بشن
WEEKDAY_MAP = [
    ("چهارشنبه", 2),
    ("سه‌شنبه", 1),
    ("سه شنبه", 1),
...
def _parse_relative_day_and_time(text: str):
    """الگوی رایج فارسی: «امروز/فردا/پس‌فردا» یا اسم روز هفته + «ساعت HH:MM». اگه پیدا بشه، دقیق و مطمئنه."""
    now = now_tehran()
    day_offset = None
    day_span = None
    ...
    if day_offset is None:
        for word, target_weekday in WEEKDAY_MAP:
            idx = text.find(word)
            if idx != -1:
                days_ahead = (target_weekday - now.date().weekday()) % 7
                if "بعد" in text:
                    days_ahead += 7
    ...
    remaining = re.sub(r"^بعد\b\s*", "", remaining).strip()
